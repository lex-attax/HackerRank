    ...
    public static void checkAddMemberCardinality(Tree t, int x) throws Exception {
        ...
        if (nT == (t.cardinality() +1)) {
            if (t.member(x)) {
                ...
                //System.out.println("All is good, out cardinality increased and the thign" +
                     //   "wasn't in the tree");
           // }


            // 2. OR the thing that was addes war already there and therefor not really added
            // so the cardinality stayed the same.
        } else if (nT == t.cardinality()) {
            if (!t.member(x)) {
                throw new Exception("The cardinality didn'1 increase by 1, but" +
                        "we added a new thing");
            } //else {
                //System.out.println("All is good, the thing was in the tree already" +
               //         "and our cardinality did'nt increase");
            //}
        } else {
